...
config = ConfigParser.SafeConfigParser({"lang": "en", "plugins" : "", "lang-in-name": "no" })
cache_folder_path = '~/.config/periscope'
...
class SubSceneTestCase(unittest.TestCase):
    def runTest(self):
        import SubScene
        subdb = SubScene.SubScene(config, cache_folder_path)
...
class SubSceneStep2TestCase(unittest.TestCase):
    def runTest(self):
        import SubScene
        subdb = SubScene.SubScene(config, cache_folder_path)
        subtitle = {'release': u'Dexter.S04E01.HDTV.XviD-NoTV', 'lang': 'ar',
                'link': None, 'page': u'http://subscene.com/arabic/dexter-fourth-season/subtitle-263042.aspx'}
